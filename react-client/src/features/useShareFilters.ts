...
export const useShareFilters = (shares: ShareDto[], shareType: 'received' | 'sent') => {
  ...
  const getStatusPriority = (share: ShareDto): number => {
    if (share.fileIsDeleted) return 4;
    if (share.used) return 3;
    const now = new Date();
    if (new Date(share.expiresAt) <= now) return 2;
    return 1; 
  };
