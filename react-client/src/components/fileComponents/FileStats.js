...
const FileStats = ({ files }) => {
    ...
    return (_jsx("div", { className: "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6", children: statItems.map((item, index) => {
            const count = stats[item.key] || 0;
            const Icon = item.icon;
            return (_jsx(motion.div, { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, transition: { delay: index * 0.1 }, className: `${item.bg} rounded-xl p-4 border border-gray-100 hover:shadow-md transition-all duration-200`, children: _jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("p", { className: "text-2xl font-bold text-gray-800", children: count }), _jsx("p", { className: "text-sm text-gray-600 font-medium", children: item.label })] }), _jsx(Icon, { className: `w-6 h-6 ${item.color}` })] }) }, item.key));
        }) }));
};
